* loads raw data from NSS51

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/Nss51_1.0/Data/"
set more off
set trace on
...
!cat `snames' > NSS51raw.txt  // combine rural and urban
...
la def itemlbl
	...
	800 "Residential building and land(Cost of repairs)"
	808 "Other Durables"
...
forvalues l=1/11 {
	infix `commonvar' `level`l'var' using NSS51raw.txt if level==`l', clear
	...
	la var ent_type			"Enterprise Type"
	...
	save "`datadir'/dta/NSS51/level`l'", replace
}
