* loads raw data from NSS52

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/Nss52_1.0/Data/"
set more off
set trace on
...
!cat `snames' > NSS52raw.txt  // combine rural and urban
...
forvalues l=1/12 {
	infix `commonvar' `level`l'var' using NSS52raw.txt if level=="0`l'", clear
	...
	save "`datadir'/dta/NSS52/level`l'", replace
}	 
