* loads raw data from NSS47

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/Nss47_1.0/Data/"
set more off
set trace on
...
!cat `snames' > NSS47raw.txt  // combine rural and urban
...
forvalues l=1/8 {
	infix `commonvar' `level`l'var' using NSS47raw.txt if level=="0`l'", clear
	gen hhid=plot_no+hhd
	...
	save "`datadir'/dta/NSS47/level`l'", replace
}
