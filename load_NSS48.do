* loads raw data from NSS48

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/Nss48_1.0/Data/"
set more off
set trace on
...
!cat `snames' > NSS48raw.txt  // combine rural and urban
...
forvalues l=1/7 {
	infix `commonvar' `level`l'var' using NSS48raw.txt if level=="0`l'", clear
	gen hhid=plot_no+sub_strat+hhd
	...
	save "`datadir'/dta/NSS48/level`l'", replace
}
