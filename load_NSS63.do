* Loading Data for NSS 63 Round

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/NSS63/Data/"
set more off
set trace on
...
forvalues s=1/8 {
	local snames "`snames'data`s'.TXT "
}
!cat `snames' > NSS63raw.txt  
...
local level7var								///
	 item_code   				41-43 	///
	 no_dos						44-46	///
	 firsthand_purchase_no		47-49 	///
	 hire_purchase   			50-50 	///
	 firsthand_purchase365  	51-58 	///
	 cost2   					59-66 	///
	 sechand_purchase_no   		67-69	///
	 sechand_purchase365   		70-77	///
	 total_expense365   		78-85
...
la def pub_worklbl
	1 "Got work in Public Works"
	2 "Sought but didnot get work in public works"
	3 "Did not seek work in public works";
	
la def complaintlbl
	1 "Complaint to Panchayat"
	2 "Complained to BDO/SDO/other Govt. official"
	3 "Knew that there was provision for complaining but did not complain"
	4 "Did not know there was provision for complaining";
...
forvalues l=1/7 {
	infix `commonvar' `level`l'var' using NSS63raw.txt if level=="0`l'", clear
	gen hhid=lot_fsu+seg_no+sec_strat+hhs
	...
	if level==3 {
		...
		la var pub_work			"Got work in Public Works"
		la var days_work		"No. of days got work"
		la var wage_cash		"Total wages-Cash"
		la var wage_kind		"Total wages-Kind"
		la var total_c_k		"Total wages-Total"
		la var complaint		"Complaint to authority"
		la var w_compensate		"Whether received compensation"
		la var comp_amt			"Amount of Compensation Received"
		la val relation rellbl
		la val sex sexlbl
		la val marital_stat mstatuslbl
		la val educ edulbl
		la val pub_work pub_worklbl
		la val complaint complaintlbl
		la val w_compensate yesnolbl
	}
	...
	if level==5 {
		la var item_code	"Items"
		la var quant_no		"Quantity of goods consumed(number)"
		la var val			"Value of goods consumed"
		la val item_code itemlbl
	}
	...
	save "`datadir'/dta/NSS63/level`l'", replace
}
