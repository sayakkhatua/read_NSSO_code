* loads raw data from NSS53

local datadir "~/Google Drive/India NSS/data"
cd "`datadir'/raw/Nss53_1.0/Data/"
set more off
set trace on
...
!cat `snames' > NSS53raw.txt  // combine rural and urban
...
forvalues l=1/12 {
	infix `commonvar' `level`l'var' using NSS53raw.txt if level=="0`l'", clear
	...
	la var town_class		"Town Class"
	la var area_type		"Area Type"
	...
	save "`datadir'/dta/NSS53/level`l'", replace
}	 
